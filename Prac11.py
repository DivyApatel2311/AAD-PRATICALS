...
# Define the graph with nodes and weights
graph = {
...
# Custom filter to display '∞' instead of float('inf')
@app.template_filter('format_inf')
def format_inf(value):
    return '∞' if value == float('inf') else value

# Route for displaying the form and results
...
def index():
    ...
    return render_template('index.html', result=result, start_node=start_node, error_message=error_message)
